...
if [ $stage -le 3 ]; then
  ...
  do
    ...
    utils/queue.pl --mem 2G -l hostname="!b03*" $EXP_DIR/log/spectral/spectral_${filename}.log \
      python diarizer/spectral/sclust.py \
          --out-rttm-dir $EXP_DIR/out_spectral \
...
if [ $stage -le 4 ]; then
  ...
  cat $EXP_DIR/out_spectral/*.rttm > $EXP_DIR/hyp.rttm
