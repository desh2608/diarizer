...
if [ $stage -le 1 ]; then
  ...
  for part in dev test; do
    kaldi_data_dir=data/callhome_2spk_${part}_kaldi
    mkdir $kaldi_data_dir
    for f in $DATA_DIR/$part/audios_8k/*.wav; do
      filename=$(basename $f .wav)
      fullpath=$(realpath $f)
      echo $filename "sox $fullpath -r 8000 -t wav - |"
...
if [ $stage -le 2 ]; then
  for part in dev test; do
    ...
    steps/segmentation/detect_speech_activity.sh --cmd "queue.pl --mem 2G -l hostname=!b02*" \
      --nj 40 --stage 0 --convert-data-dir-to-whole false --segment-padding 0.3 --acwt 0.5 \
      --graph_opts "--min-silence-duration=0.02 --min-speech-duration=0.1 --max-speech-duration=20.0" \
