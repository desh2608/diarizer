...
def get_args():
    ...
    parser.add_argument(
        "--label", type=str, default="overlap", help="Label for the overlap segments"
    )
    parser.add_argument(
        "input_rttm",
        nargs="?",
    ...
    for token in sorted_tokens:
        if token[0] == "BEG":
            ...
            if spkr_count == 1:
                ovl_end = token[1]
                overlap_segs.append(
                    Segment(reco_id, ovl_begin, end_time=ovl_end, spk_id=label)
                )
    ...
    for token in sorted_tokens:
        if token[0] == "BEG":
            ...
            if len(running_spkrs) == 1:
                ...
                single_speaker_segs.append(
                    Segment(reco_id, seg_begin, end_time=token[1], spk_id=cur_spkr)
                )
        elif token[0] == "END":
            try:
...
def main():
    ...
    segments = []
    for line in args.input_rttm:
        parts = line.strip().split()
        segments.append(
            Segment(parts[1], float(parts[3]), dur=float(parts[4]), spk_id=parts[7])
        )
    ...
    final_segs = sorted(
        overlap_segs + single_speaker_segs, key=lambda x: (x.reco_id, x.start_time)
    )
