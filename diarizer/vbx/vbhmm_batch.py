...
# Same as vbhmm.py but performs VBx on a batch of recordings instead of a single recording.
...
if __name__ == "__main__":
    ...
    for fn in file_names:
        ...
        for file_name, segs in recit:
            ...
            # Overlap assignment
            if overlap_rttm and args.init.endswith("VB") and q.shape[1] > 1:
                starts2, ends2, out_labels2 = merge_adjacent_labels(
                    start, end, labels2nd
                )
                # Keep the 2nd speaker segments which are in the overlap RTTM
                starts2, ends2, out_labels2 = get_overlapping_segments(
                    starts2, ends2, out_labels2, overlap_rttm
                )
            ...
            with open(os.path.join(args.out_rttm_dir, f"{fn}.rttm"), "w") as fp:
