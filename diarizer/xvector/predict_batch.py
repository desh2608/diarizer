...
# This script is similar to predict.py, but is meant to be used for cases when the number
# of recordings is large, so that each job extracts x-vectors for a subset of the recordings.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out-ark-dir", required=True, type=str, help="output embedding directory"
    )
    parser.add_argument(
        "--out-seg-dir", required=True, type=str, help="output segments directory"
    )
    parser.add_argument(
        "--backend",
        required=False,
    ...
    with torch.no_grad():
        for fn in file_names:
            seg_file = open(os.path.join(args.out_seg_dir, fn + ".seg"), "w")
            ark_file = open(os.path.join(args.out_ark_dir, fn + ".ark"), "wb")
                    ...
                    if (
                        ...
                        for start in range(0, slen - seg_len, seg_jump):
                            ...
                            if np.isnan(xvector).any():
                                ...
                                seg_start = round(
                                    labs[segnum, 0] / float(samplerate) + start / 100.0,
                                    3,
                                )
                                seg_end = round(
